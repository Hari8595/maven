...
Both applications will run on a single, well-secured AWS server using Docker containers. The applications keep their existing technology — Next.js (frontend), Node.js (backend) and MySQL (database) — so no change is required to the applications themselves. Redis has been added to support the employee dashboard and the WhatsApp messages sent through Twilio. As per the Team Leader's instruction, both applications share a single MySQL database.
...
For full transparency, the figures below use today's exchange rate of approximately ₹95 to 1 US Dollar (verified on 15 June 2026), and are based on the current AWS Mumbai pricing confirmed for June 2026. I have separated what is already running from what is planned, and I have been careful to include every item — including a few that were added as the work progressed — so that the picture is complete and there are no surprises later. The rupee value will move slightly with the exchange rate, as AWS bills in US Dollars.
...
---
*Allzone Management Services · Confidential — For Review and Approval · June 2026*
