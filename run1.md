...
# Build-time heap ceiling set inline on the build command (memory-heavy step)
...
GRANT SELECT, INSERT, UPDATE, DELETE ON emp360_db.* TO 'app_user'@'%';
GRANT SELECT, INSERT, UPDATE, DELETE ON rec360_db.* TO 'app_user'@'%';
FLUSH PRIVILEGES;
```
...
```yaml
# ═══════════════════════════════════════════════════════════════════
# Production Stack — CIS/OWASP hardened, 24/7 self-healing
...
- **Node heap (`NODE_OPTIONS`):** build-time 1536 MB on the build command (frontend build is heavy), runtime 512 MB per app (keeps MySQL/Redis healthy on an 8 GB box). All adjustable.
